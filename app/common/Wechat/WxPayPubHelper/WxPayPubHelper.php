...
class Common_util_pub
{
	...
	public function getSign($Obj)
	{
		...
		//签名步骤二：在string后加入KEY
		$String = $String."&key=".Config::get('app.wx')['key'];
	...
	{		
        //初始化curl        
       	$ch = curl_init();
		//设置超时
		curl_setopt($ch, Config::get('app.wx')['curl_timeout'], $second);
        //这里设置代理，如果有的话
	...
	function postXmlSSLCurl($xml,$url,$second=30)
	{
		...
		curl_setopt($ch,CURLOPT_SSLCERT, Config::get('app.wx')['sslcert_path']);
		//默认格式为PEM，可以注释
		curl_setopt($ch,CURLOPT_SSLKEYTYPE,'PEM');
		curl_setopt($ch,CURLOPT_SSLKEY, Config::get('app.wx')['sslkey_path']);
		//post提交方式
		curl_setopt($ch,CURLOPT_POST, true);
		curl_setopt($ch,CURLOPT_POSTFIELDS,$xml);
		$data = curl_exec($ch);
...
class Wxpay_client_pub extends Common_util_pub 
{
	...
	function createXml()
	{
	   	$this->parameters["appid"] = Config::get('app.wx')['appid'];//公众账号ID
	   	$this->parameters["mch_id"] = Config::get('app.wx')['mchid'];//商户号
...
{	
	function __construct() 
	{
		...
		//设置curl超时时间
		$this->curl_timeout = Config::get('app.wx')['curl_timeout'];
	}
	...
	function getPrepayId()
	{
		$this->postXml();
		$this->result = $this->xmlToArray($this->response);
		return empty($this->result["prepay_id"]) ? '' : $this->result["prepay_id"];
	}
...
class NativeCall_pub extends Wxpay_server_pub
{
	...
	function createXml()
	{
		if($this->returnParameters["return_code"] == "SUCCESS"){
		   	$this->returnParameters["appid"] = Config::get('app.wx')['appid'];//公众账号ID
		   	$this->returnParameters["mch_id"] = Config::get('app.wx')['mchid'];//商户号
...
class JsApi_pub extends Common_util_pub
{
	...
	{
		$urlObj["appid"] = Config::get('app.wx')['appid'];
	...
	{
		$urlObj["appid"] = Config::get('app.wx')['appid'];
		$urlObj["secret"] = Config::get('app.wx')['appsecret'];
	...
	function getOpenid()
	{
        ...
        //初始化curl
       	$ch = curl_init();
		//设置超时
		curl_setopt($ch, Config::get('app.wx')['curl_timeout'], $this->curl_timeout);
		curl_setopt($ch, CURLOPT_URL, $url);
        curl_setopt($ch,CURLOPT_SSL_VERIFYPEER,FALSE);
        curl_setopt($ch,CURLOPT_SSL_VERIFYHOST,FALSE);
		curl_setopt($ch, CURLOPT_HEADER, FALSE);
	...
	{
		$jsApiObj["appId"] = Config::get('app.wx')['appid'];
		$timeStamp = time();
