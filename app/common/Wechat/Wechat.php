...
<Nonce><![CDATA[%s]]></Nonce>
    ...
    protected function setCache($cachename,$value,$expired){
        return Cache::add($cachename, $value, $expired / 60);
    }
    ...
    public function checkAuth($appid='',$appsecret='',$token=''){
        ...
        if ($rs = $this->getCache($authname))  {
            ...
            Log::info('access_token(cacehd): '.$this->access_token);
            return $rs;
        }
        ...
        if ($result)
        {
            ...
            if (!$json || isset($json['errcode'])) {
                ...
                Log::info('error - get access_token');
                return false;
            }
    ...
    public function getJsTicket($appid='',$jsapi_ticket=''){
        ...
        if ($rs = $this->getCache($authname))  {
            ...
            Log::info('jsapi_ticket(cacehd): '.$this->jsapi_ticket);
            return $rs;
        }
        ...
        if ($result)
        {
            ...
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
                $this->errMsg = $json['errmsg'];
                Log::info('error - get jsapi_ticket');
                return false;
            }
            $this->jsapi_ticket = $json['ticket'];
            $expire = $json['expires_in'] ? intval($json['expires_in'])-100 : 3600;
            $this->setCache($authname,$this->jsapi_ticket,$expire);
            Log::info('jsapi_ticket: '.$this->jsapi_ticket);
